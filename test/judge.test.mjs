...
const tmpCfg = () => ({ ...loadConfig(join(PROJECT_ROOT, "config.json")), __cachePath: join(mkdtempSync(join(tmpdir(), "ag-")), "c.json") });
...
test("MCP 豁免 server 直接 allow", async () => {
  const cfg = { ...tmpCfg(), fastpath: { ...DEFAULTS.fastpath, mcp_allowlist: ["lark-cli"] } };
