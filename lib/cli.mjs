...
export async function runSetup(flags, io = defaultIo()) {
  try {
    ...
    const rcPath = join(homedir(), ".agentguardrc");
    writeRc(rcPath, key);
    console.log(`[1/4] key 已写入 ${rcPath}`);
    if (!flags.noVerify) {
      const v = await verifyKey(key, loadConfig());
      console.log(v.ok ? "[1/4] key 验证通过(真实 API)" : `[1/4] key 验证失败(${v.type}),已写入,可稍后用 agent-guard check 复测`);
    }
    ...
    if (flags.dryRun) { console.log("dry-run:未写入任何文件"); return; }
