...
export function render(level, reason, cfg) {
  if (level === "allow")
    return { stdout: JSON.stringify(buildOutput("allow", `[agent-guard] 安全通过(${reason})`)) };
  if (level === "confirm" && !cfg.degrade_ask_to_deny)
    return { stdout: JSON.stringify(buildOutput("ask", `[agent-guard] 需人工确认(${reason})`)) };
  const head = level === "block" ? "高危禁止" : "需人工确认";
  const text = `[agent-guard] ${head}(${reason})。此操作需人工确认后才可执行:请停下向用户说明,等待用户自行执行或明确同意;不要重试、不要改写命令规避本拦截。`;
  return { stdout: JSON.stringify(buildOutput("deny", text)) };
}
